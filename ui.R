...
shinyUI(fluidPage(
     titlePanel("R軟體套件Shiny在互動式資訊圖表的應用"),
	helpText("銘傳大學應用統計資訊學系"),
	helpText("指導：陳明輝 副教授"),
	helpText("學生：鄭易承 李訓豪 林瑋倫 曾少宏 歐芳如"),

navlistPanel(

	"UKDriverDeaths",
	tabPanel("UKDriverDeaths資料",
	...
	tabPanel("圓餅圖呈現", 
		...
		selectInput("testp","年份：",
				choices=rownames(qqall),
                   	selected="1975"),
      mainPanel(
        plotOutput("testp_Plot")

	)),

	tabPanel("直條圖呈現",

		radioButtons("bar","平均/最大值/最小值：",
				choices=colnames(qqsmm),
				selected="mean"),
	mainPanel(
		plotOutput("bar_Plot")
	))
